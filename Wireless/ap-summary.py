...
import threading
import clean_file_ap as clean_file
...
tempoutput1 = '_ap_summary.txt'
tempoutput2 = '_ap_summary_clean.csv'
spreadsheet_name = timestamp + '_WLC_AP_Summary.xlsx'
...
def deviceconnector(i,q):
    ...
    while True:
        ...
        result1 = net_connect.send_command(ap_sum_cmd)
        # Write the result to the temp text file and close the file
        Temp_TXT_1.write(result1)
        Temp_TXT_1.close()

        # Clean up the AP summary file to only include the 
        # AP names, call the clean file function
        clean_file.clean_file(ip_address,tftp_path)
        
        # Load in the cleaned text file
        df1 = pd.read_csv(str(tftp_path) + ip_address + tempoutput2, header=None)
         
        # Create new sheet as name of the IP Address
        workbook.create_sheet(ip_address)
        # Set the active worksheet to current IP Address
        worksheet=workbook[ip_address]

        df1_list = []  # Create List Variable
        df1_list = df1[0].tolist()

        curr_row = 1  # Set the current row to 2 to start writing data from this row
        ...
        os.remove(tftp_path + ip_address + tempoutput1)
        os.remove(tftp_path + ip_address + tempoutput2)
...
def main():
    ...
    # Save the workbook
    workbook.save(tftp_path_final + spreadsheet_name)
    print('*** WLC AP Summary Completed ***')
